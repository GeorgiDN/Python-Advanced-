...
def check_for_email_symbol(email, symbol="@"):
    if email.count(symbol) != 1:
        raise MustContainAtSymbolError(f"Email must contain {symbol}")


def check_name_length(name):
    if len(name) <= MIN_LENGTH_NAME:
        raise NameTooShortError(f"Name must be more than 4 characters")


def check_is_valid_domain(domain):
...
def validate_email():
    while True:
        ...
        check_for_email_symbol(email)

        name = email.split("@")[0]
        check_name_length(name)

        domain = email.split(".")[-1]
        check_is_valid_domain(domain)
