...
def play(board, player=1):
    while True:
        field_check_for_draw = []
        for r in range(DEFAULT_ROWS_COUNT):
            for c in range(DEFAULT_COLUMNS_COUNT):
                field_check_for_draw.append(board[r][c])
        if 0 not in field_check_for_draw:
            print("Draw")
            break
