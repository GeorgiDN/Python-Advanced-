...
def create_bunker(cat_info):
    category = {cat: {"items": [], "qty": [], "qly": []} for cat in cat_info.split(', ')}

    return category


def fill_bunker(cat_info, num_fills):
    for _ in range(num_fills):
        ...
        cat_info[category]["items"].append(item)
        cat_info[category]["qty"].append(qty_value)
        cat_info[category]["qly"].append(qly_value)
...
bunker = fill_bunker(create_bunker(categories_data), number_of_fillings)
